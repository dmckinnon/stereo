...
This algorithm works in a very similar way to how we [find a homography between the images](https://github.com/dmckinnon/stitch#finding-the-best-transform). When finding a homography - a transform between two planes - between images, we find at least 8 pairs of matching points *x* and *x'* and plug them into the equation we want: *H* *x* = *x'*, or *H* *x* - *x'* = __0__, where *H* is the homography we want. For each pair, we can rewrite this as three equations in three unknowns for each row of *H* ... or, since they all equate to zero, we can add them together to make one equation in nine unknowns: *h1* *x1* + ... -*x'1* - *x'2* - *x'3* = 0, where *h1*, etc are the values in the matrix *H*. We then create a system of linear equations *A* *h* = 0, where *h* = (*h1*, *h2* ... *h9*), and the rows of *A* are the coefficients from the above equations, *x1*, etc. To solve this for *h*, and from that get *H*, we use what's called [Singular Value Decomposition](https://en.wikipedia.org/wiki/Singular_value_decomposition), or SVD for short (you'll see this in the code - it's quicker to type). This is not a simple thing to understand, so don't feel bad about just moving past it and thinking only "it's a process to solve systems of linear equations". Point is, we use this to solve for *H* here. How does that relate to what we are doing to fund the fundamenal matrix? 
...
So let's go through the process of computing the __rectified__ images. There are different ways of doing it that each depend on how much information you have. If you only have the Fundamental matrix, and no camera calibration matrix, then the ever-popular [Zhang's method](http://dev.ipol.im/~morel/Dossier_MVA_2011_Cours_Transparents_Documents/2011_Cours7_Document2_Loop-Zhang-CVPR1999.pdf) is needed. This method also does not assume you have no lens distortion, and computes an anti-distortion model, a projective transform, a similarity, and a shear. I will warn that this is a difficult paper and nontrivial to go through, but it is succinct and straightforward - no fluff or obscurity. I'll go over it briefly. 

Another method I know relies on knowing the extrinsic calibration between the cameras - either from being given the transform between the cameras, or by computing it from knowing the fundamental matrix and having the camera calibration matrices. I'll go over this first, and I implement this in code, as this is the simpler method. 


### The Physical method - camera calibration and fundamental matrix, or given essential matrix
So what we need is a pair of rotations that transform the cameras into an ideal stereo setup - where the cameras are pointing the same way, and are only a translation in *x* apart. If this is hard to understand, imagine as above the cameras bolted on to the edge of a table, both facing out and parallel. You know how far apart they are and there is no rotation between them any more. 

Let's compute this. First, we know the rotation currently between the cameras from the extrinsics. Let's assume that the first camera is at the origin of our world coordinate system and facing forward along the *z* axis, so that the only rotation we consider is the one of the second camera from the first - call this *R*. Apply 1/2 *R* to each camera, so that they are parallel. Why 1/2 *R* and not just rotate the second camera by *R*? Because of image warp. This rotation to the image will warp it considerably, so we minimise the warp to each image by sharing the rotation around. 

Secondly, we now need to reorient the cameras such that the baseline - the translation between the cameras - is now the common *x*-axis. To use our analogy from before of the table, we might have started out with each camera sitting on the table, both tilted towards some common object. We twisted them both to point straight forward (and magically warped the whole world so that they are still looking at the object), and now we are rotating the world of the table to make the edge of the table - an axis for us - along the line between the cameras. 
